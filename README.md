...
In each of these, cd navigate to the project folder.
...
ngrok start server (my custom setup)

Update the .env file. After starting ngrok, it will provide a new URL. Copy this URL and paste it into the .env file of the designer-extension.

Configure the data-client .env. Ensure the .env file in the data-client directory has the proper credentials for your application.

### Ngrok preflight

To ensure ngrok functions correctly, a preflight request is necessary on api calls.

```javasript
const response = await fetch(
    `https://url-to-ngrok.web`,
    {
        method: "GET",
        headers: {
...
> [!Important] > **Refresh your browser!** Until we make some changes to the designer, you'll need to refersh your browser to see changes from the API take effect. 🙇🏾‍♀️
